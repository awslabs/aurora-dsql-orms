...
/** Unit tests for AuroraDSQLDatabaseType. These tests don't require a database connection. */
class AuroraDSQLDatabaseTypeTest {
  ...
  void handlesAwsDsqlUrlWithoutPostgresqlPrefix() {
    assertTrue(
        databaseType.handlesJDBCUrl("jdbc:aws-dsql://abc123.dsql.us-east-1.on.aws/postgres"));
  }
  ...
  void doesNotHandleStandardPostgresqlUrl() {
    assertFalse(databaseType.handlesJDBCUrl("jdbc:postgresql://localhost:5432/mydb"));
  }
  ...
  void doesNotHandleMysqlUrl() {
    assertFalse(databaseType.handlesJDBCUrl("jdbc:mysql://localhost:3306/mydb"));
  }

  @Test
  @DisplayName("Should NOT handle jdbc:oracle:// URL")
  void doesNotHandleOracleUrl() {
    assertFalse(databaseType.handlesJDBCUrl("jdbc:oracle:thin:@localhost:1521:xe"));
  }
  ...
  void returnsCorrectDriverClassForDsqlUrl() {
    String driverClass =
        databaseType.getDriverClass(
            "jdbc:aws-dsql:postgresql://abc123.dsql.us-east-1.on.aws/postgres", null);
  ...
  void returnsPostgresDriverClassForTransformedUrl() {
    String driverClass =
        databaseType.getDriverClass(
            "jdbc:postgresql://abc123.dsql.us-east-1.on.aws:5432/postgres", null);
