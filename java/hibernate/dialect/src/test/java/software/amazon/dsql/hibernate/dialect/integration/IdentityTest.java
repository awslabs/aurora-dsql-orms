...
public class IdentityTest extends DSQLHibernateBaseTest {
    ...
    void testLongIdentityPrimaryKey() {
        ...
        try (Session session = getSession()) {
            IdentityKeyLongEntity retrievedEntity = session.get(IdentityKeyLongEntity.class, entity.getId());
            assertEquals("Identity Entity 1", retrievedEntity.getName());
        }
    }

    /**
     * DSQL only supports IDENTITY with bigint, so this test confirms entities with int ID handle casting correctly.
     */
    @Test
    void testIntIdentityPrimaryKey() {
        ...
        try (Session session = getSession()) {
            IdentityKeyIntEntity retrievedEntity = session.get(IdentityKeyIntEntity.class, entity.getId());
            assertEquals("Identity Entity 1", retrievedEntity.getName());
