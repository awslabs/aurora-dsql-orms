...
create table if not exists pets (birth_date date, id UUID DEFAULT gen_random_uuid() not null, owner_id UUID REFERENCES owners (id), type_id UUID REFERENCES types (id), name varchar(255), primary key (id));
create table if not exists vet_specialties (specialty_id UUID not null REFERENCES specialties (id), vet_id UUID not null REFERENCES vets (id), primary key (specialty_id, vet_id));
create table if not exists visits (visit_date date, id UUID DEFAULT gen_random_uuid() not null, pet_id UUID REFERENCES pets (id), description varchar(255) not null, primary key (id));
