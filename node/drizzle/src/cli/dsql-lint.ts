...
function findDsqlLint(): string {
  ...
  // On Windows a `cargo install` produces `dsql-lint.exe`; match it directly.
  // We only add `.exe` (a spawnable PE), not `.cmd`/extensionless npm shims,
  // which spawnSync (no shell) rejects with EINVAL since CVE-2024-27980.
  const binaryName =
  ...
  if (output.schema_version !== SUPPORTED_SCHEMA_VERSION) {
    throw new Error(
      `dsql-lint JSON schema_version ${output.schema_version} is not supported ` +
        ...
        `Align versions: install a compatible dsql-lint, or update @aws/aurora-dsql-drizzle.`,
    );
  }
  return output;
}
