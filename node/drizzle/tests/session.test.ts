...
describe("AwsDsqlDatabase.transactionWithRetry", () => {
  ...
  test("validates the retryConfig up front", async () => {
    const db = drizzle({ query: jest.fn() } as never) as AwsDsqlDatabase;
    const callback = jest.fn(async () => "x");
    // maxRetries < 0 must reject before the callback ever runs.
    await expect(
      db.transactionWithRetry(callback, undefined, { maxRetries: -1 }),
    ).rejects.toThrow(/maxRetries must be an integer >= 0/);
