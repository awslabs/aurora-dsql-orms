...
`;
  ...
  describe("sequence support", () => {
    test("passes with warning when autoincrement() is used", async () => {
      const schema = `
datasource db {
  provider     = "postgresql"
  relationMode = "prisma"
...
`;
    ...
    test("no warning for Int @id without autoincrement", async () => {
      const schema = `
datasource db {
  provider     = "postgresql"
  relationMode = "prisma"
...
`;
    ...
    test("no warning for BigInt @id", async () => {
      const schema = `
datasource db {
  provider     = "postgresql"
  relationMode = "prisma"
...
`;
      ...
      expect(
        result.issues.some((i) =>
          i.suggestion?.includes("GENERATED BY DEFAULT AS IDENTITY"),
        ),
      ).toBe(true);
    });
