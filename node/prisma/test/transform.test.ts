...
/**
 * Executes a query with OCC retry logic.
 * DSQL DDL modifies the schema catalog; back-to-back DDL from different
 * transactions can hit OC001 ("schema has been updated by another transaction").
 */
async function queryWithRetry(
  ...
  for (let attempt = 0; ; attempt++) {
    try {
      ...
      const isOcc = pgErr.code === "40001" || pgErr.message?.includes("OC001");
...
describe("Migration Transformer", () => {
  ...
  afterAll(async () => {
    if (dsql) {
      for (const table of [...tablesToCleanup].reverse()) {
        await queryWithRetry(dsql, `DROP TABLE IF EXISTS "${table}"`).catch(
          () => {},
        );
      }
      await dsql.end();
  ...
  async function executeOnDsql(
    ...
    for (const t of tables) {
      ...
      await queryWithRetry(dsql, `DROP TABLE IF EXISTS "${t}"`);
    }
    for (const stmt of extractStatements(transformedSql)) {
      await queryWithRetry(dsql, stmt);
    }
  }
...
CREATE INDEX "idx2" ON "user"("name");`;
    ...
    test("preserves non-FK ALTER TABLE statements", async () => {
      ...
      if (dsql) {
        ...
        await queryWithRetry(dsql, `DROP TABLE IF EXISTS "${table}"`);
        await queryWithRetry(
          dsql,
