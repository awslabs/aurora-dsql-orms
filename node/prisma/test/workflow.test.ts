...
`;
    ...
    test("schema with autoincrement passes validation with warning", async () => {
      const schema = `
datasource db {
  provider     = "postgresql"
  relationMode = "prisma"
...
`;
      ...
      expect(
        result.issues.some(
          (i) => i.type === "warning" && i.message.includes("hot keys"),
        ),
      ).toBe(true);
    });

    test("autoincrement full pipeline: validate schema then transform SERIAL SQL", async () => {
      // Step 1: Schema with autoincrement passes validation
      const schema = `
datasource db {
  provider     = "postgresql"
  relationMode = "prisma"
...
`;
      const validationResult = await validateSchema(createTempSchema(schema));
      expect(validationResult.valid).toBe(true);

      // Step 2: Simulated Prisma migrate diff output (Prisma generates SERIAL for autoincrement)
...
CREATE TABLE "User" (
    ...
    CONSTRAINT "User_pkey" PRIMARY KEY ("id")
      ...
      const transformResult = transformMigration(prismaSql, {
        includeHeader: false,
      });

      // SERIAL should be converted to BIGINT IDENTITY with CACHE
      expect(transformResult.sql).toContain(
        "BIGINT GENERATED BY DEFAULT AS IDENTITY (CACHE 1)",
      );
      expect(transformResult.sql).not.toMatch(/\bSERIAL\b/);
      expect(transformResult.stats.serialTypesConverted).toBe(1);

      // Should still be wrapped in transaction
      expect(transformResult.sql).toContain("BEGIN;");
      expect(transformResult.sql).toContain("COMMIT;");
    });
  });
