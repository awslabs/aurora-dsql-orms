...
from sqlalchemy import bindparam, select, sql, Integer, BIGINT
...
class AuroraDSQLDDLCompiler(PGDDLCompiler):
    ...
    def visit_create_sequence(self, create, **kw):
        """Override to add CACHE parameter required by DSQL and convert INTEGER to BIGINT"""
        # Convert INTEGER data_type to BIGINT since DSQL doesn't support INTEGER sequences
    ...
    def get_column_specification(self, column, **kw):
        ...
        if has_sequence:
            ...
            colspec += " " + self.type_compiler.process(column.type, type_expression=column)
            ...
            colspec += " DEFAULT nextval('%s')" % self.preparer.format_sequence(sequence)
        ...
        if is_identity:
            ...
            if hasattr(column, 'identity') and column.identity and column.identity.cache is not None:
...
class AuroraDSQLDialect(PGDialect):
    ...
    def do_execute(self, cursor, statement, parameters, context=None):
        """Execute statement and commit immediately if DDL since DSQL doesn't support multiple DDL in one transaction"""
    ...
    def do_execute_no_params(self, cursor, statement, context):
        """Execute DDL and commit immediately since DSQL doesn't support multiple DDL in one transaction"""
